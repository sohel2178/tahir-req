...
export default function LiveAnimatedMap({ device }: Props) {
  ...
  const move = (geo: RedisGeo) => {
    ...
    const fromSpeed = Number(lastGeo.current?.speed || 0);
    const toSpeed = Number(geo.speed || 0);
    ...
    const animate = (ts: number) => {
      ...
      setSpeed(currentSpeed); // 👈 ADD THIS
  ...
  return (
    ...
    >
      ...
      <Marker
        position={position}
        onClick={() => setShowInfo(true)} // 👈 ADD THIS
      ...
      {/* {lastGeo.current && device && (
        <VehicleInfo
          title="Vehicle Info"
          geo={data[data.length - 1]}
          device={device}
        />
      )} */}

      {showInfo && lastGeo.current && device && (
        <VehicleInfoWindow
          position={position}
          device={device}
          geo={lastGeo.current}
